...
title_text = "Maxwell model: sinusoidal strain (Lissajous curve)"
...
ax.text(0.5, 0.25, var_text, transform=ax.transAxes)
...
ax.text(0.5, 0.05, res_text, transform=ax.transAxes)

lissajous, = plt.plot([], [], 'r', animated=True)
# ここでは[],[]としているが、下で***.set_data([0, l + x[i]], [0, 0])で実際の値を入れている

samp_template = r'$\sigma_{{amp}}$ = %.3f MPa'
samp_text = ax.text(0.35, 0.9, '', transform=ax.transAxes)
pdiff_template = r'$\theta$ = %.1f$\degree$'
pdiff_text = ax.text(0.35, 0.8, '', transform=ax.transAxes)
...
def animate(i):
    lissajous_length = 200
    current_time = t_ani[i]

    if i < lissajous_length:
        lissajous.set_data([e[:i]],[s[:i]])
    else:
        lissajous.set_data([e[i-lissajous_length:i]],[s[i-lissajous_length:i]])
    ...
    return lissajous, samp_text, pdiff_text, af_text, aft_text, time_text

# アニメーション実行   
ani = animation.FuncAnimation(fig, animate, frames=len(t_ani), interval=interval_ms, blit=False, repeat=False)

savefile = './mp4/Maxwell_sinuStrain_Lissajous.mp4'

# SLS IIモデルの常微分方程式（振動歪み）（アニメーション付き）
...
ax.grid()
title_text = "SLS II model: sinusoidal strain"
...
ax.text(0.8, 0.28, '$\epsilon$ (input)', transform=ax.transAxes)
ax.text(0.8, 0.18, '$\sigma$ (output)', transform=ax.transAxes)
...
ax.text(0.8, 0.42, samp_text, transform=ax.transAxes)
phase_diff_text = r'$\theta$ = {0:.1f} $\degree$'.format(phase_diff)
ax.text(0.8, 0.35, phase_diff_text, transform=ax.transAxes)
...
def init():               # FuncAnimationでinit_funcで呼び出す
    time_text.set_text('')
    return bar, rod1, rod2, rod3, point, triangle1, rod_da, damper, triangle2, strain_bar, arrow_e_p, arrow_e_n, stress_bar, arrow_s_p, arrow_s_n, time_text
...
savefile = './mp4/SLS2_sinuStrain_ani_(f={0:.2f}Hz).mp4'.format(freq)

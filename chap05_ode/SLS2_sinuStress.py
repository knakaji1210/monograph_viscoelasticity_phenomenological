...
try:
    ...
    samp = 0.02*10**6
try:
    freq = float(input('frequency for sinusoidal stress [Hz] (default=0.2653 Hz): '))
except ValueError:
    freq = 5/(6*np.pi)
...
# ODEの解析
e0 = 0                              # 初期条件として定義
sol = odeint(SLS2_sinuStress, e0, t_post, args=(samp, af, insMod, k, tau))
...
ax1.set_title("SLS II model: sinusoidal stress")
ax1.grid(True, ls='--')
...
line_strain, = ax2.plot([], [], color='blue', lw=2, label='Step strain (t = {0:.1f} s)'.format(event_time))
...
ax2.text(0.1, 0.35, eamp_text, transform=ax2.transAxes)
...
savefile = './mp4/SLS2_sinuStress_(f={0:.2f}Hz).mp4'.format(freq)

# SLS Iモデルの常微分方程式（振動応力）（アニメーション付き）
...
ax.grid()
title_text = "SLS I model: sinusoidal stress"
...
ax.text(0.85, 0.28, '$\sigma$ (input)', transform=ax.transAxes)
ax.text(0.85, 0.18, '$\epsilon$ (output)', transform=ax.transAxes)
...
ax.text(0.85, 0.42, eamp_text, transform=ax.transAxes)
...
def init():               # FuncAnimationでinit_funcで呼び出す
    time_text.set_text('')
    return bar, rod, point, rod_sp1, rod_sp2, triangle1, triangle2, rod_da, damper, stress_bar, arrow_s_p, arrow_s_n, strain_bar, arrow_e_p, arrow_e_n, time_text
...
savefile = './mp4/SLS1_sinuStress_ani_(f={0:.2f}Hz).mp4'.format(freq)

...
try:
    ...
    eamp = 0.1
try:
    freq = float(input('frequency for sinusoidal strain [Hz] (default=1.59 Hz): '))
except ValueError:
    freq = 5/np.pi
...
# ODEの解析
s0 = 0                              # 初期条件として定義
sol = odeint(SLS2_sinuStrain, s0, t_post, args=(eamp, af, insMod, k, tau))
...
ax1.set_title("SLS II model: sinusoidal strain")
ax1.grid(True, ls='--')

line_strain, = ax1.plot([], [], color='blue', lw=2, label='Step strain (t = {0:.1f} s)'.format(event_time))
line_strain_e1, = ax1.plot([], [], color='cyan', ls="dashed", lw=1, label='$\epsilon$ (spring 1 in Maxwell)')
line_strain_e2, = ax1.plot([], [], color='gray', ls="dashed", lw=1, label='$\epsilon$ (dashpot in Maxwell)')
...
line_stress_s1, = ax2.plot([], [], color='green', ls="dashed", lw=1, label='$\sigma$ (Maxwell)')
line_stress_s2, = ax2.plot([], [], color='orange', ls="dashed", lw=1, label='$\sigma$ (spring 2)')
#line_stress_s1_d, = ax2.plot([], [], color='black', ls="dotted", lw=1, label='$\sigma$ (dashpot in Maxwell)')
...
eq_text = r'd$\sigma$/d$t$ = ($E_\infty$$\epsilon$ + $E_i$$\tau$ d$\epsilon$/d$t$ - $\sigma$)/$\tau$'
...
#    line_stress_s1_d.set_data(t[:i], s1_d[:i])

    return line_strain, line_strain_e1, line_strain_e2, line_stress, line_stress_s1, line_stress_s2
#    return line_strain, line_strain_e1, line_strain_e2, line_stress, line_stress_s1, line_stress_s2, line_stress_s1_d

# アニメーション実行   
ani = animation.FuncAnimation(fig, animate, frames=steps, interval=interval_ms, blit=True)

savefile = './mp4/SLS2_sinuStrain_(f={0:.2f}Hz).mp4'.format(freq)

# relaxation modulus of SLS I model

import numpy as np
from scipy.optimize import curve_fit
import matplotlib.pyplot as plt

def relaxMod(E, k, tau, t_elapsed):
    relaxMod = np.where(t_elapsed >= 0, E*(1/k + (1 - 1/k)*np.exp(-k*t_elapsed/tau)), 0)
    # E must be insMod
    return relaxMod

def relaxFunc(k, tau, t_elapsed):
...
def reqParams():
    ...
    k = insMod/infMod
    relaxTime = eta/E2              # [s] relaxation time
...
if __name__=='__main__':
    ...
    idx1 = np.abs(x1_scaled - 1/k).argmin()
    x1_label = r'$t$/$\tau$'
    x2 = timeAxis[2]
    x2_scaled = timeAxis[3]
    idx2 = np.abs(x2_scaled - np.log(1/k)).argmin()
    x2_label = r'log[$t$/$\tau$]'

    if select == 0:
        y1 = relaxMod(insMod, k, relaxTime, x1)  # E = insMod
        y1 /= 10**6             # rescale to MPa
        y1_label = r'$E$($t$) /MPa'
        label1 = r'Rleaxtion modulus (linear)'
        relaxMod = relaxMod(insMod, k, relaxTime, x2)
        ...
        savefile = './png/SLS1_relaxation modulus.png'

    if select == 1:
        y1 = relaxFunc(k, relaxTime, x1)
        y1_label = r'$\phi$($t$)'
        label1 = 'Relaxation function (linear)'
        relaxFunc = relaxFunc(k, relaxTime, x2)
        ...
        savefile = './png/SLS1_relaxation_func.png'
    ...
    ax1.vlines(1/k, 0, y1[idx1], color='k', ls='dashed')
    ax1.set_axisbelow(True)
    ax1.text(0.3, 0.8, res_text, transform=ax1.transAxes)
    ...
    ax2.vlines(np.log(1/k), 0, y2[idx2], color='k', ls='dashed')

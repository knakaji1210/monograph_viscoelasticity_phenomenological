...
def MittagLeffler_self(z, terms):
    """パラメータ a=2, b=1 の Mittag-Leffler 関数を級数和で計算"""
...
if __name__=='__main__':
    ...
    if select == 0:     # exp(x)の場合
        a = 1.0  # Mittag-Leffler関数のパラメータ
        b = 1.0   # Mittag-Leffler関数のパラメータ
        # X軸のデータ点を生成        
        x_vals = t_vals  # exp(x)のためにt=xとする        
        # 各計算の実行
        y_orig = np.exp(t_vals)
        y_ML = MittagLeffler(a, b, x_vals, num_terms=50)
        savefile = './png/MittagLeffler_exp.png'
    elif select == 1:   # cosh(x)の場合
        a = 2.0  # Mittag-Leffler関数のパラメータ
        b = 1.0   # Mittag-Leffler関数のパラメータ
        # X軸のデータ点を生成
        ...
        y_ML = MittagLeffler(a, b, x_vals, num_terms=50, ignore_special_cases=True)
#        y_ML = MittagLeffler_self(x_vals, terms=50)  # 自作関数（MittagLeffler_self）で計算
        ...
        y_ML = t_vals * MittagLeffler(a, b, x_vals, num_terms=50, ignore_special_cases=True)
    ...
    ax.plot(t_vals, y_ML, label=f"Mittag-Leffler Function, $a={a}$, $b={b}$", color="gray", linestyle="-")

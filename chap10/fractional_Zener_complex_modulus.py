...
def freqAxes(retardTime):
    ...
    angFreq = np.logspace(int(np.log10(centerAngFreq))-9, int(np.log10(centerAngFreq))+5, 57)
...
def fitAngFreqs():
    try:
        minAngFreq_s = float(input('Enter minimum frequency for fitting (storage) (default = -4): '))
    except ValueError:
        minAngFreq_s = -4
    try:
        maxAngFreq_s = float(input('Enter maximum frequency for fitting (storage) (default = -1): '))
    except ValueError:
         maxAngFreq_s = -1
    try:
        minAngFreq_l = float(input('Enter minimum frequency for fitting (loss) (default = -8): '))
    except ValueError:
        minAngFreq_l = -8
    try:
...
if __name__=='__main__':
    ...
    param_text = """
    ($E_i$ = {0:.1f} MPa, $E_{{\infty}}$ = {1:.1f} MPa, $E$ = {2:.1f} MPa, $\\tau$ = {3:.1f} ms, $\\nu$ = {4:.2f})""".format(insMod/10**6, infMod/10**6, modulus/10**6, relaxTime*10**3, nu)
    ...
    strMod = complexMod(insMod, k, relaxTime, angFreq, nu)[0]
    losMod = complexMod(insMod, k, relaxTime, angFreq, nu)[1]
    ...
    if select == 2:
        ...
        savefile = './png/fractional_Zener_loss_tangent_nu{:.2f}.png'.format(nu)

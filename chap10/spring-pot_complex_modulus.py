...
def freqAxes(relaxTime):
    ...
    angFreq = np.logspace(int(np.log10(centerAngFreq))-3, int(np.log10(centerAngFreq))+3, 43)
    scaledAngFreq = angFreq*relaxTime
    freqAxes = [angFreq, scaledAngFreq]
    return freqAxes

def calc_complexMod(E, tau, af, nu):
    comMod = E*(tau*af*(2j/2))**nu
    strMod = comMod.real
    losMod = comMod.imag
...
def fitAngFreqs():
    try:
        minAngFreq_s = float(input('Enter minimum frequency for fitting (storage) (default = -2): '))
    except ValueError:
        minAngFreq_s = -2
    try:
        maxAngFreq_s = float(input('Enter maximum frequency for fitting (storage) (default = 2): '))
    except ValueError:
         maxAngFreq_s = 2
    try:
        minAngFreq_l = float(input('Enter minimum frequency for fitting (loss) (default = -2): '))
    except ValueError:
        minAngFreq_l = -2
    try:
        maxAngFreq_l = float(input('Enter maximum frequency for fitting (loss) (default = 2): '))
    except ValueError:
        maxAngFreq_l = 2
...
if __name__=='__main__':
    ...
    # calculating complex modulus and loss tangent
    modulus, relaxTime, nu = reqParams()
    param_text = r'($E$ = {0:.1f} MPa, $\tau$ = {1:.2f} ms, $\nu$ = {2:.2f})'.format(modulus/10**6, relaxTime*10**3, nu)
    ...
    if select == 0:
        ...
        savefile = './png/spring-pot_complex_modulus_linear_nu{:.2f}.png'.format(nu)
    ...
    if select == 1:
        ...
        a = 1
        legend_loc='upper left'
        savefile = './png/spring-pot_complex_modulus_log_nu{:.2f}.png'.format(nu)
    ...
    if select == 2:
        ...
        savefile = './png/spring-pot_loss_tangent_nu{:.2f}.png'.format(nu)

...
if __name__=='__main__':
    ...
    print('tau = {0:.1f} ms, tau_prime = {1:.1f} ms'.format(tau*10**3, tau_prime*10**3))
    param_text = """($E_i$ = {0:.1f} MPa, $E_{{\infty}}$ = {1:.1f} MPa, $E$ = {2:.1f} MPa, 
    $\\tau$ = {3:.1f} ms, $\\tau^\prime$ = {4:.1f} ms, $\\nu$ = {5:.2f})""".format(insMod/10**6, infMod/10**6, modulus/10**6, tau*10**3, tau_prime*10**3, nu)
    freqAxes = freqAxes(tau_prime)  # tauではなくtau_primeに対してomega*tau_prime=1でE"がピークを迎えるはず
    fitting = -1
    try:

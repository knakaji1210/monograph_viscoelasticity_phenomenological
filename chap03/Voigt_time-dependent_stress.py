...
def func_Voigt1(modulus, retardTime):
    ...
    stress = np.array([c*t for t in tim])
    strain = np.array([c*(10**6/modulus)*(t -retardTime * (1 - np.exp(-t/retardTime))) for t in tim])
    return c, t1, tim, stress, strain

def func_Voigt2(modulus, retardTime, c, t1):
    try:
    ...
    stress = np.array([c*t1 for t in tim])
    strain = np.array([c*(10**6/modulus)*(t1 -retardTime * (1 - np.exp(-t1/retardTime))) for t in tim])
    return t2, tim, stress, strain

def func_Voigt3(modulus, retardTime, c, t1, t2):
    try:
    ...
    stress = np.array([c*(t1 + t2 - t) for t in tim])
    strain = np.array([c*(10**6/modulus)*(t1 -retardTime * (1 - np.exp(-t1/retardTime)) + retardTime * (1-np.exp(-(t-t2)/retardTime)) - (t - t2)) for t in tim])
    return t3, tim, stress, strain
    
if __name__=='__main__':
    E, T = reqParams()
    c, t1, tim1, stress1, strain1 = func_Voigt1(E, T)
    t2, tim2, stress2, strain2 = func_Voigt2(E, T, c, t1)
    t3, tim3, stress3, strain3 = func_Voigt3(E, T, c, t1, t2)
